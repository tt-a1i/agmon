...
**技术栈：** Go 1.26 · Bubbletea TUI · SQLite (modernc, 纯 Go) · Unix socket
...
| collector/cost | `internal/collector/cost.go` | Claude 费用估算 |
| storage | `internal/storage/` | SQLite schema、读写、查询 |
| tui | `internal/tui/model.go` | Bubbletea TUI，4 个 tab |
...
**Schema 变更规则：** `migrate()` 只用 `CREATE TABLE IF NOT EXISTS` 和 `CREATE INDEX IF NOT EXISTS`，不支持 ALTER TABLE。新增字段必须是可选列，或新建表。不允许删除/重命名已有列。
...
`~/.codex/sessions/*.jsonl`，每行：`{"timestamp":"...","type":"session_meta|response_item|event_msg","payload":{...}}`
...
4 个 tab（`internal/tui/model.go`）：

| Tab | 常量 | 数据来源 |
|-----|------|---------|
| Dashboard | `tabDashboard` | `db.ListSessions()` |
| Agent Tree | `tabAgentTree` | `db.ListAgents(selectedSession)` |
| Tool Calls | `tabToolCalls` | `db.ListToolCalls(selectedSession, 100)` |
...
- [x] 僵尸 Session → daemon 启动时 MarkStaleSessionsEnded(24h)
- [x] Token 重复计数（重启后）→ source_id 唯一索引 + INSERT OR IGNORE
- [x] COST 列恒显示 $0.00 → 改为 IN/OUT token 列
